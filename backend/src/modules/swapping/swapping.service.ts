...
export class SwappingService {
  ...
  async initializeBattery(firstSwapDto: FirstSwapDto) {
    ...
    try {
      this.logger.log(
        `Initializing first battery swap for user ID ${user_id}, vehicle ID ${vehicle_id}`,
      );
      return await this.databaseService.$transaction(async (prisma) => {
        // ✅ FIXED: assignBatteryToVehicle now updates both Battery AND Vehicle
        // No need to call updateBatteryId separately
        const updatedBattery = await this.batteriesService.update(
          taken_battery_id,
          {
            station_id: null,
            cabinet_id: null,
        ...
        this.logger.log(
          `Battery initialization completed for user ID ${user_id}, vehicle ID ${vehicle_id}`,
        );
        return {
  ...
  private async findFirstEmptySlot(station_id: number) {
    ...
    if (!cabinets?.length) {
      throw new BadRequestException(
        'No active cabinets available at this station.',
      );
    }
    ...
    throw new BadRequestException(
      'No empty slots available in the cabinets at this station.',
    );
  }

  async getEmptySlotForReturnBattery(dto: FindEmptySlotDto) {
    const { user, vehicle, subscription } =
  ...
  async returnBatteryToCabinet(dto: ReturnBatteryDto) {
    ...
    if (!vehicle.battery_id) {
      this.logger.log(
        `Vehicle ID ${vehicle.vehicle_id} has no battery to return.`,
      );
    ...
    const returnBattery = await this.batteriesService.findOne(
      vehicle.battery_id,
    );

    if (returnBattery.soh.lessThan(new Decimal(80))) {
      throw new BadRequestException(
        'Battery SOH is below 80%. Cannot return to cabinet.',
      );
    }
    ...
    const batteryStatus = returnBattery.current_charge.equals(new Decimal(100))
      ? BatteryStatus.full
      : BatteryStatus.charging;

    this.logger.log(
      `Returning battery ${returnBattery.battery_id} to cabinet ${dto.cabinet_id}, slot ${dto.slot_id}. ` +
        `Charge: ${returnBattery.current_charge}% -> Status: ${batteryStatus}`,
    );

    // Perform all updates within a transaction
    return await this.databaseService.$transaction(async (prisma) => {
      // Execute updates in parallel within transaction
      const [updatedBattery, updatedVehicle, updatedSubscription, swapRecord] =
        await Promise.all([
          //Update battery to cabinet slot
          this.batteriesService.update(
            returnBattery.battery_id,
            {
          ...
          this.subscriptionsService.updateDistanceTraveled(
            subscription.subscription_id,
            returnBattery.current_charge.toNumber(),
            prisma,
          ),
  ...
  async getTakenBatterySlot(dto: FindBatteryFullDto) {
    ...
    const reservation =
      await this.reservationsService.findOneScheduledForVehicleByUserId(
        dto.user_id,
        dto.vehicle_id,
      );
    ...
    if (reservation && reservation.battery_id) {
      ...
      takenBattery = await this.batteriesService.findOne(
  ...
  async takeBatteryFromCabinet(dto: TakeBatteryDto) {
    ...
    const [takenBattery, existingSwapTransaction, reservation] =
      await Promise.all([
        this.batteriesService.findOne(dto.taken_battery_id),
        this.swapTransactionsService.findPendingTransactionByVehicle(
          dto.vehicle_id,
        ),
        this.reservationsService.findOneScheduledForVehicleByUserId(
          dto.user_id,
          dto.vehicle_id,
        ),
      ]);
    ...
    if (!reservation && takenBattery.status !== BatteryStatus.full) {
      throw new BadRequestException(
        'Only batteries with FULL status can be taken without a reservation.',
      );
    }

    if (reservation && takenBattery.battery_id !== reservation.battery_id) {
      throw new BadRequestException(
        'The taken battery does not match the reserved battery.',
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        'Battery is not in the specified station/cabinet.',
      );
    }
    ...
    return await this.databaseService.$transaction(async (prisma) => {
      ...
      const [
        ...
        this.subscriptionsService.incrementSwapUsed(
          subscription.subscription_id,
          prisma,
        ),
      ]);
